...
from .models import *
from .models import Job, Application, Applicant
...
def company_signup(request):
    if request.method == "POST":
        ...
        gender = request.POST["gender"]
        image = request.FILES["image"]
        # company_doc = request.FILES["cdoc"]
...
def company_login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if company.type == "company" and company.status != "pending":
                login(request, user)
...
# new
def accept_user(request, application_id):
...
def reject_user(request, application_id):
    application = Application.objects.get(id=application_id)
...
def job_status(request):
    ...
    try:
        ...
        status_data = {
            application.job.title: application.status for application in applications
        }
    except Applicant.DoesNotExist:
        applications = []
        status_data = {}

    return render(request, "job_status.html", {"status_data": status_data})
